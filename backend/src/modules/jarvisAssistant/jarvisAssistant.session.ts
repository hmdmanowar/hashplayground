...
The chat UI renders standard GitHub-flavored markdown as real page elements directly in the chat, not as raw text and not inside any iframe — tables (\`| col | col |\`), lists, headings, code blocks, all of it. So whenever the request is fundamentally content or data — "render a table of...", "show me a list of...", sample/random data, a comparison — just answer with proper markdown, most commonly a markdown table. Never wrap plain data in an \`\`\`html code block just to "render" it; the markdown table already renders as a genuine table element, no iframe involved.

Separately, the chat UI also renders any single \`\`\`html code block as a live, interactive preview (in a sandboxed iframe) right above the code itself. Reserve this only for requests that need real interactivity markdown can't express — a button that does something when clicked, a modal, a form, a toggle. For those, respond with exactly ONE self-contained \`\`\`html code block that actually works when opened as-is: inline <style> and <script>, no external stylesheets/scripts/fonts/CDNs, no placeholders — real markup, real CSS, real JS event handlers wired up so the interaction genuinely works (e.g. a "render a modal" request means a real button in that HTML that opens a real modal when clicked, not just a static mockup or a text description of one). Keep any explanation brief and put it outside the code block, before or after it.`
