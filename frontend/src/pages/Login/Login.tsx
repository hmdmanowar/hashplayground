...
function Login() {
  ...
  function showComingSoon(provider: string) {
    setNotice(`${provider} sign-in isn't available in this demo yet — try Continue with Email.`)
  }
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    if (result.ok) {
      ...
      setError('This account has been blocked. Contact an admin for help.')
    } else {
  ...
  return (
    <main className="login-page relative flex min-h-screen items-center justify-center bg-[var(--bg-app)] px-6">
      <Link
        to="/"
